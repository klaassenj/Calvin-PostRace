  ...
  render: function () {
    var Header = (
      <div id="header">
        ...
        <h2>
          {this.state.meet} {this.state.event}{" "}
        </h2>
      </div>
    );
    ...
    var SubmitDate = this.createCard(
      "Date Submitted: ",
      moment(this.state.date, "x").format("LLLL")
    );
    return (
      <div>
        <div className="container">
          <span>
            <button
              className="navButton"
              onClick={() => this.navigate("pastraces")}
            >
              Back
            </button>
            <button className="navButton" onClick={this.editRace}>
              Edit
            </button>
          </span>
          {Header}
